Shader "Custom/unityCookieBeguinner/03_specular_vertex_lit" {
    ...
    SubShader {
        Pass {
            ...
            vertexOutput vert(vertexInput vi)
            {
                ...
                float3 view_dir = normalize(_WorldSpaceCameraPos.xyz - mul(_Object2World, vi.vertex).xyz);
                // rotation of light source
                float3 light_dir = normalize(_WorldSpaceLightPos0.xyz);
            	// calculated diffuse reflection dot product
                float3 diffuse_prod = max(0.0, dot(normal_dir, light_dir));
                ...
                float3 spec_ref = _Atten * _SpecColor.rgb * diffuse_prod * 
                    pow(
                        max(
                            0.0, 
                            dot(reflect(-light_dir, normal_dir), view_dir)
                       ),
                        _Shininess
                   );
                ...
                vo.clr = float4(light_specular, 1.0);
